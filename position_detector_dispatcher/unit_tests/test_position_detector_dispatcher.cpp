...
#define SIZE_TEST_EVENT_PACKET 4096
	...
	std::string to_string(const std::wstring& wstr)
	{
		...
		std::string narrow_str = converter.to_bytes(wstr);
	...
	public:
		...
		TEST_METHOD(test_dispatcher_process_incoming_message)
		{
			checked_execute([]
			{
				...
				position_detector_dispatcher::settings_func_t settings_func([&sync_ip, &events_ip, sync_port, events_port](const std::string &key)->std::vector<std::string>
				{
				...
				std::thread syncro_server_thread([&synchro_server](){synchro_server.start_server<test_synchro_packet_t>(gen_next_syncro_packet, count_messages, 10000); });
