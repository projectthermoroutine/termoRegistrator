    ...
    public class metro_db_controller
    {
        public metro_db_controller(metro_db_controller controller)
        {
            if (controller == null)
            {
               ...
               // TODO
               // Dictionary<string, Dictionary<int,IEnumerable<Registrator.DB.ResultEquipCode>>> line_path_objects_array = new Dictionary<string,Dictionary<int,IEnumerable<ResultEquipCode>>>();

                //line_path_objects_array.Add("key", new Dictionary<int, IEnumerable<ResultEquipCode>>());
        ...
        public void setLineAndPath(string line, string path)
        {
            ...
            int path_ = Convert.ToInt32(path);

            if(line != currentLine || path_ != current_path)
            {
                currentLine = line;
                current_path = path_;

                int lineNumber = get_line_number(line);
                
                if(lineNumber != -1)
                {
                    retrieve_groups();
                    get_objects(lineNumber, path_);
                }
                else
                {
                    //error
